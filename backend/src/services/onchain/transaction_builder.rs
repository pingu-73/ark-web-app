...
use bitcoin::{
    absolute::LockTime, transaction::Version, Address, AddressType, Amount, FeeRate, OutPoint,
...
impl TransactionBuilder {
    ...
    pub async fn build_and_broadcast(
        ...
        let (tx, _change_amount) = self
            .build_transaction(available_utxos, to_address, amount, fee_rate)
            .await?;
    ...
    pub async fn estimate_fee(
        ...
        let (_, fee, _) = self
            .calculate_transaction_details(available_utxos, to_address, amount, fee_rate)
            .await?;
    ...
    pub async fn estimate_fee_with_estimates(
        ...
        for priority in priorities {
            ...
            let fee_rate =
                FeeRate::from_sat_per_vb(fee_rate).ok_or_else(|| anyhow!("Invalid fee rate"))?;
    ...
    async fn calculate_transaction_details(
        ...
        let estimated_fee = Amount::from_sat(
            fee_rate
                .fee_vb(estimated_size as u64)
        ...
        let mut actual_fee = Amount::from_sat(
            fee_rate
                .fee_vb(actual_size as u64)
                .expect("Fee calculation failed")
                .to_sat(),
        );
        ...
        if actual_fee < min_fee {
            tracing::info!(
                "Increasing fee from {} to {} to meet minimum relay fee",
                actual_fee,
                min_fee
            );
            actual_fee = min_fee;
    ...
    async fn sign_transaction(
        ...
        for (input_index, utxo) in selected_utxos.iter().enumerate() {
            ...
            match utxo.address.address_type() {
                ...
                Some(address_type) => {
                    return Err(anyhow!(
                        "Unsupported address type for signing: {:?}",
                        address_type
                    ));
                }
                None => {
        ...
        tracing::info!(
            "Successfully signed transaction with {} inputs",
            selected_utxos.len()
        );
        Ok(())
    }
