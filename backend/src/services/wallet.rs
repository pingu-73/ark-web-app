...
use crate::services::onchain::{FeeEstimator, OnChainPaymentService};
...
pub async fn get_wallet_info() -> Result<WalletInfo> {
    ...
    let network = std::env::var("BITCOIN_NETWORK").unwrap_or_else(|_| "regtest".into());
    let server_url =
        std::env::var("ARK_SERVER_URL").unwrap_or_else(|_| "http://localhost:7070".into());
...
pub async fn estimate_onchain_fee_detailed(
    ...
    for (priority, priority_name, blocks) in vec![
        ...
        if let Ok(total_fee) = payment_service
            .transaction_builder
...
pub async fn get_vtxo_list() -> Result<Vec<serde_json::Value>> {
    ...
    match client {
        Some(client) => match client.spendable_vtxos().await {
            Ok(vtxos) => {
                let vtxo_list: Vec<serde_json::Value> = vtxos
                    .iter()
                    .map(|(outpoints, vtxo)| {
