...
impl SubscriptionManager {
    ...
    pub async fn subscribe_to_scripts(
        &self,
    ) -> Result<Pin<Box<dyn Stream<Item = ScriptUpdate> + Send>>> {
        // [TODO!!]
    ...
    pub async fn monitor_vtxo_changes(
        &self,
    ) -> Result<Pin<Box<dyn Stream<Item = VtxoUpdate> + Send>>> {
        let (tx, rx) = mpsc::channel(100);
        let grpc_client = self.grpc_client.clone();
    ...
    async fn check_script_updates(
        grpc_client: &Arc<crate::services::ark_grpc::ArkGrpcService>,
    ) -> Result<Vec<ScriptUpdate>> {
        // [TODO!!!]
        Ok(Vec::new())
    }

    async fn get_current_vtxo_state(
        grpc_client: &Arc<crate::services::ark_grpc::ArkGrpcService>,
    ) -> Result<std::collections::HashMap<String, VtxoInfo>> {
    ...
    pub async fn subscribe_to_balance_changes(
        &self,
    ) -> Result<Pin<Box<dyn Stream<Item = BalanceUpdate> + Send>>> {
        let (tx, rx) = mpsc::channel(100);
        let grpc_client = self.grpc_client.clone();
