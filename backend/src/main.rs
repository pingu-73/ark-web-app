...
async fn main() {
    ...
    let ark_server_url =
        std::env::var("ARK_SERVER_URL").unwrap_or_else(|_| "http://localhost:7070".to_string());

    let wallet_manager = Arc::new(services::multi_wallet::MultiWalletManager::new(
        db_manager.clone(),
        network,
        ark_server_url,
    ));
    ...
    let faucet_service = Arc::new(services::faucet::FaucetService::new(esplora_url, network));
    ...
    let app = Router::new()
        ...
        .route(
            ...
            get(api::multi_wallet::get_wallet_info),
        )
        // Balance endpoints
        .route(
            "/api/wallets/:wallet_id/balance",
            get(api::multi_wallet::get_wallet_balance),
        )
        .route(
            "/api/wallets/:wallet_id/balance/onchain",
            get(api::multi_wallet::get_onchain_balance),
        )
        .route(
            "/api/wallets/:wallet_id/balance/offchain",
            get(api::multi_wallet::get_offchain_balance),
        )
        // Off-chain operations
        .route(
            "/api/wallets/:wallet_id/send-vtxo",
            post(api::multi_wallet::send_vtxo),
        )
        .route(
            "/api/wallets/:wallet_id/vtxos",
            get(api::multi_wallet::get_vtxo_list),
        )
        .route(
            "/api/wallets/:wallet_id/participate-round",
            post(api::multi_wallet::participate_in_round),
        )
        // On-chain operations
        .route(
            "/api/wallets/:wallet_id/send-onchain",
            post(api::multi_wallet::send_onchain),
        )
        .route(
            "/api/wallets/:wallet_id/fee-estimates",
            get(api::multi_wallet::get_fee_estimates),
        )
        .route(
            "/api/wallets/:wallet_id/estimate-fee",
            post(api::multi_wallet::estimate_transaction_fee),
        )
        // Transaction history
        .route(
            "/api/wallets/:wallet_id/transactions",
            get(api::multi_wallet::get_transaction_history),
        )
        // Faucet
